<?php
    ...
    function ns_basics_page_layout_meta_box($post) {
        ...
        $banner_title = isset( $values['rypecore_banner_title'] ) ? esc_attr( $values['rypecore_banner_title'][0] ) : '';
        $banner_text = isset( $values['rypecore_banner_text'] ) ? esc_attr( $values['rypecore_banner_text'][0] ) : '';
        ...
        $banner_text_align = isset( $values['rypecore_banner_text_align'] ) ? esc_attr( $values['rypecore_banner_text_align'][0] ) : $banner_text_align_default;
        ...
        $banner_shortcode = isset( $values['rypecore_banner_shortcode'] ) ? esc_attr( $values['rypecore_banner_shortcode'][0] ) : '';

        //cta values
        $cta_display = isset( $values['rypecore_cta_display'] ) ? esc_attr( $values['rypecore_cta_display'][0] ) : '';
        $cta_title = isset( $values['rypecore_cta_title'] ) ? esc_attr( $values['rypecore_cta_title'][0] ) : '';
        $cta_text = isset( $values['rypecore_cta_text'] ) ? esc_attr( $values['rypecore_cta_text'][0] ) : '';
        ...
        if(isset($_GET['post_type']) && $_GET['post_type'] == 'job_listing') {
            $page_layout_default = get_option('rypecore_job_detail_default_layout', 'right sidebar');
        } else {
            $page_layout_default = 'full';
        }
        $page_layout_widget_area_default = 'Page_sidebar';

        $page_layout = isset( $values['rypecore_page_layout'] ) ? esc_attr( $values['rypecore_page_layout'][0] ) : $page_layout_default;
        $page_layout_widget_area = isset( $values['rypecore_page_layout_widget_area'] ) ? esc_attr( $values['rypecore_page_layout_widget_area'][0] ) : $page_layout_widget_area_default;
                ...
                <table class="admin-module">
                    <tr>
                        ...
                        <td class="admin-module-field">
                            ...
                            <tr>
                            <td><input type="radio" name="rypecore_page_layout" id="page_layout_full" value="full" <?php if($page_layout == 'full') { echo 'checked="checked"'; } ?> /></td>
                            ...
                            <table class="left">
                            <tr>
                            <td><input type="radio" name="rypecore_page_layout" id="page_layout_right_sidebar" value="right sidebar" <?php if($page_layout == 'right sidebar') { echo 'checked="checked"'; } ?> /></td>
                            ...
                            <table class="left">
                            <tr>
                            <td><input type="radio" name="rypecore_page_layout" id="page_layout_left_sidebar" value="left sidebar" <?php if($page_layout == 'left sidebar') { echo 'checked="checked"'; } ?> /></td>
    ...
    function ns_basics_save_page_layout_meta_box( $post_id )
    {
        ...
        if( isset( $_POST['rypecore_banner_display'] ) ) {
            update_post_meta( $post_id, 'rypecore_banner_display', wp_kses( $_POST['rypecore_banner_display'], $allowed ) );
        } else {
            update_post_meta( $post_id, 'rypecore_banner_display', wp_kses( '', $allowed ) );
        }

        if( isset( $_POST['rypecore_banner_source'] ) )
            update_post_meta( $post_id, 'rypecore_banner_source', wp_kses( $_POST['rypecore_banner_source'], $allowed ) );
            
        if( isset( $_POST['rypecore_banner_title'] ) )
            update_post_meta( $post_id, 'rypecore_banner_title', wp_kses( $_POST['rypecore_banner_title'], $allowed ) );

        if( isset( $_POST['rypecore_banner_text'] ) )
            update_post_meta( $post_id, 'rypecore_banner_text', wp_kses( $_POST['rypecore_banner_text'], $allowed ) );
            
        if( isset( $_POST['rypecore_banner_bg_img'] ) )
            update_post_meta( $post_id, 'rypecore_banner_bg_img', wp_kses( $_POST['rypecore_banner_bg_img'], $allowed ) );
            
        if( isset( $_POST['rypecore_banner_bg_display'] ) )
            update_post_meta( $post_id, 'rypecore_banner_bg_display', wp_kses( $_POST['rypecore_banner_bg_display'], $allowed ) );

        if( isset( $_POST['rypecore_banner_overlay'] ) ) {
            update_post_meta( $post_id, 'rypecore_banner_overlay', wp_kses( $_POST['rypecore_banner_overlay'], $allowed ) );
        } else {
            update_post_meta( $post_id, 'rypecore_banner_overlay', wp_kses( '', $allowed ) );
        }
        
        if( isset( $_POST['rypecore_banner_overlay_opacity'] ) )
            update_post_meta( $post_id, 'rypecore_banner_overlay_opacity', wp_kses( $_POST['rypecore_banner_overlay_opacity'], $allowed ) );
            
        if( isset( $_POST['rypecore_banner_overlay_color'] ) )
            update_post_meta( $post_id, 'rypecore_banner_overlay_color', wp_kses( $_POST['rypecore_banner_overlay_color'], $allowed ) );

        if( isset( $_POST['rypecore_banner_text_align'] ) )
            update_post_meta( $post_id, 'rypecore_banner_text_align', wp_kses( $_POST['rypecore_banner_text_align'], $allowed ) );

        if( isset( $_POST['rypecore_banner_padding_top'] ) )
            update_post_meta( $post_id, 'rypecore_banner_padding_top', wp_kses( $_POST['rypecore_banner_padding_top'], $allowed ) );

        if( isset( $_POST['rypecore_banner_padding_bottom'] ) )
            update_post_meta( $post_id, 'rypecore_banner_padding_bottom', wp_kses( $_POST['rypecore_banner_padding_bottom'], $allowed ) );

        if( isset( $_POST['rypecore_banner_slider_cat'] ) )
            update_post_meta( $post_id, 'rypecore_banner_slider_cat', wp_kses( $_POST['rypecore_banner_slider_cat'], $allowed ) );

        if( isset( $_POST['rypecore_banner_slider_layout'] ) )
            update_post_meta( $post_id, 'rypecore_banner_slider_layout', wp_kses( $_POST['rypecore_banner_slider_layout'], $allowed ) );

        if( isset( $_POST['rypecore_banner_slider_num'] ) )
            update_post_meta( $post_id, 'rypecore_banner_slider_num', wp_kses( $_POST['rypecore_banner_slider_num'], $allowed ) );

        if( isset( $_POST['rypecore_banner_shortcode'] ) )
            update_post_meta( $post_id, 'rypecore_banner_shortcode', wp_kses( $_POST['rypecore_banner_shortcode'], $allowed ) );

        if( isset( $_POST['rypecore_page_layout'] ) )
            update_post_meta( $post_id, 'rypecore_page_layout', wp_kses( $_POST['rypecore_page_layout'], $allowed ) );

        if( isset( $_POST['rypecore_page_layout_container'] ) ) {
            update_post_meta( $post_id, 'rypecore_page_layout_container', wp_kses( $_POST['rypecore_page_layout_container'], $allowed ) );
        } else {
            update_post_meta( $post_id, 'rypecore_page_layout_container', wp_kses( '', $allowed ) );
        }

        if( isset( $_POST['rypecore_page_layout_widget_area'] ) )
            update_post_meta( $post_id, 'rypecore_page_layout_widget_area', wp_kses( $_POST['rypecore_page_layout_widget_area'], $allowed ) );

        if( isset( $_POST['rypecore_cta_display'] ) ) {
            update_post_meta( $post_id, 'rypecore_cta_display', wp_kses( $_POST['rypecore_cta_display'], $allowed ) );
        } else {
            update_post_meta( $post_id, 'rypecore_cta_display', wp_kses( '', $allowed ) );
        }
            
        if( isset( $_POST['rypecore_cta_title'] ) )
            update_post_meta( $post_id, 'rypecore_cta_title', wp_kses( $_POST['rypecore_cta_title'], $allowed ) );

        if( isset( $_POST['rypecore_cta_text'] ) )
            update_post_meta( $post_id, 'rypecore_cta_text', wp_kses( $_POST['rypecore_cta_text'], $allowed ) );

        if( isset( $_POST['rypecore_cta_button_text'] ) )
            update_post_meta( $post_id, 'rypecore_cta_button_text', wp_kses( $_POST['rypecore_cta_button_text'], $allowed ) );

        if( isset( $_POST['rypecore_cta_button_url'] ) )
            update_post_meta( $post_id, 'rypecore_cta_button_url', wp_kses( $_POST['rypecore_cta_button_url'], $allowed ) );

        if( isset( $_POST['rypecore_cta_bg_img'] ) )
            update_post_meta( $post_id, 'rypecore_cta_bg_img', wp_kses( $_POST['rypecore_cta_bg_img'], $allowed ) );
            
        if( isset( $_POST['rypecore_cta_bg_display'] ) )
